    ...
    <article className="tw-post">
      ...
      <div className="tw-post-body">
        <div className="tw-post-header">
          ...
          <span className="tw-post-meta">
            · {formatRelativeTime(post.created_at)}
            {post.client_label && ` · via ${post.client_label}`}
          </span>
        </div>
        ...
        <div className="tw-post-actions">
          ...
          <button className="tw-action" type="button" onClick={toggleLike} disabled={!viewerAccountId || likeBusy} title={viewerAccountId ? undefined : 'Pick a character to act as first'}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill={viewerHasLiked ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={viewerHasLiked ? 'tw-heart-filled' : ''}>
